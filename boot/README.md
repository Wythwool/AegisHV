...
`scripts/build-type1-kernel.sh` builds the minimal `x86_64-unknown-none` kernel ELF to `target/type1/aegishv-type1.elf`. The ELF writes the early success marker only after the minimal Limine handoff fields, including HHDM offset and executable address bases, are present; otherwise it writes a separate missing-handoff marker and halts. It is not packaged as a bootable ISO yet.
