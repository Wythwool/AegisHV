...
fn boot_artifacts_define_limine_linker_and_entry_boundaries() {
    ...
    assert_contains_all(
        ...
        &[
            ...
            ".double_fault_guard ALIGN(4K) (NOLOAD)",
            ".vmx_exit_guard ALIGN(4K) (NOLOAD)",
            ".boot_stack_guard ALIGN(4K) (NOLOAD)",
            ...
            "kernel exceeds its owned paging window",
            "host page-table pool size changed",
            "boot-stack guard must precede its stack",
        ],
    );
    assert_contains_all(
        &entry,
