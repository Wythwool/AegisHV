...
fn linux_vmi_profile_doc_records_synthetic_only_scope() {
    ...
    assert_contains_all(
        &text,
        &[
            "synthetic/offline Linux x86_64 profile metadata format",
            "does not add Linux guest inspection",
            "No real Linux kernel profile data ships by default.",
            "P079 handles KASLR base resolution later.",
            "It does not load real kernel symbols.",
            "It does not resolve KASLR.",
            "It does not walk `task_struct`, modules, or syscall tables.",
