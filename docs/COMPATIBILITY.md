...
| Intel VMX toy-guest runtime | VMXON/VMCS/EPT, a finite preemption probe, trap-all I/O A/B pages, a fixed MSR bitmap that permits only direct `RDMSR IA32_PAT`, exact bitmap/PAT VMCS readback, synthetic `IA32_EFER`, and fixed PAT/x87/SIMD guard stages are implemented in the lab kernel; reviewed hardware execution evidence is not present |
...
A source build, model test, or TCG boot is not Intel VMX execution coverage. That claim requires matching valid pre/post-run SHA-256 image digests, the complete strict sixteen-marker chain, and a validated CPU/timer diagnostic set from a recorded nested-VMX or bare-metal configuration. Even that evidence covers only the fixed deadline probe, contained accesses through both I/O bitmap pages, CPUID, one synthetic high-read RDMSR response, the deliberate PAT entry/save/restore/readback sequence, exact `#NM` exits for fixed `FNOP` and `MOVDQA`-self probes, and HLT. It does not qualify selective bitmap policy, general MSR or exception virtualization, WRMSR, FPU/SIMD context switching, or production use.
