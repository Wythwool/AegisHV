...
- The live Intel path allocates fifteen distinct pages, including immutable trap-all I/O A/B and MSR pages, and requires exact VMCS bitmap-address readback. Its fixed payload suppresses writes to ports `0xe9` and `0x8000`, executes CPUID, receives synthetic zero for trapped `RDMSR IA32_EFER`, and halts; it never replays guest I/O or MSR on the host. A general guest loader, reusable lifecycle, multiple guests, and guest crash recovery are not implemented.
- PAT, XSAVE/FPU state, comprehensive stateful MSR context, WRMSR, selective/dynamic I/O and MSR bitmap policy, broad exit coverage, and hostile-guest recovery are incomplete.
