...
The separate `aegishv-type1-kernel` target is a bootable x86_64 lab path, not a backend for the userspace sensor. It wires one BSP-only Intel VMX toy guest, a single fifteen-page allocation ledger, immutable trap-all I/O A/B pages, a fixed MSR read allowlist with checked VMCS addresses, and a bounded owned CR3 for that final path. The only untrapped MSR operation is a direct guest read of `IA32_PAT`; all writes and other reads trap. The fixed guest exercises two contained port exits, one synthetic `IA32_EFER` response, a narrow VMCS PAT transition/readback, and exact `#NM` exits for one x87 and one SIMD no-effect probe. Complete early/dynamic/per-CPU paging, general PAT and FPU/SIMD context management, stateful MSR/WRMSR handling, selective bitmap policy, SMP, interrupt, device/IOMMU, lifecycle, and hardware-evidence gates remain open.
