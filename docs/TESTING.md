...
`scripts/build-type1-kernel.sh` builds the `x86_64-unknown-none` kernel ELF and writes `target/type1/aegishv-type1-kernel-build.txt`. It requires the Rust `x86_64-unknown-none` target, builds with static relocation and the kernel code model, and records `bootable_image=false` and `qemu_evidence=false` because the ELF alone is not a Limine boot image or runtime proof. The kernel validates the Limine base revision and bounded HHDM, memory-map, and executable-address response state before using the handoff. It installs and verifies owned GDT/TSS/IDT state, captures CPU/MSR capabilities, validates CR0/CR4 fixed bits and VMX controls, and builds one early allocation ledger. That ledger excludes the linked kernel span and inherited active CR3 root before supplying VMXON/VMCS/guest/EPT pages from Limine `USABLE` memory; the same owner reaches the guest path. After all HHDM writes, the final Intel path requires NX/four-level paging, enables NXE/WP, materializes and reads back a four-page root, flushes inherited global translations, switches CR3, and validates live RX/R/RW mappings plus five absent stack guards. On an eligible Intel host it writes the complete toy-guest VMCS, executes VMLAUNCH with an initial zero-value VMX preemption-timer sentinel, then resumes a finite TSC-or-count probe with a reload derived from a hard `0x01000000`-TSC-tick budget and `IA32_VMX_MISC`. The effective deadline cannot exceed the requested budget, and reloads below 2 are refused. Only a real nonzero timer expiration advances RIP to the port-I/O/CPUID/HLT payload and emits the preemption marker. The probe reaches the same fallback HLT at either a `0x08000000`-TSC-tick horizon or a `0x01000000`-iteration limit; an HLT or timer exit at that exact RIP emits `guest-timeout` and regains control, while other unexpected probe exits emit `guest-exit-error`. Later payload stages remain bounded. The handler validates and suppresses the expected port-I/O exit, handles CPUID and HLT, and executes VMXOFF. Failed handoff, host-table, host-paging, runtime, deadline, entry, exit, and resume checks emit explicit failure markers.
...
`scripts/type1-qemu-smoke.sh` is opt-in lab plumbing for the bootable Limine ISO. It deliberately refuses a raw ELF because QEMU `-kernel` does not provide the required Limine handoff. By default, success requires the complete serial lines `aegishv:type1:host-tables-ok`, `aegishv:type1:backend-vmx`, `aegishv:type1:vmxon-cycle-ok`, `aegishv:type1:vmcs-load-ok`, `aegishv:type1:host-paging-ok`, `aegishv:type1:guest-config-ok`, `aegishv:type1:guest-preempt-exit-ok`, `aegishv:type1:guest-io-exit-ok`, `aegishv:type1:guest-cpuid-exit-ok`, `aegishv:type1:guest-hlt-exit-ok`, and `aegishv:type1:guest-run-ok` in that order. A log is rejected if it also contains a contradictory backend, host-table, host-paging, or runtime failure, skipped VMX operation, `aegishv:type1:guest-timeout`, guest entry/exit/resume error, exception, missing-Limine, or panic marker. The script defaults to `q35,accel=kvm` and `host,+vmx`; `AEGISHV_QEMU_MACHINE` and `AEGISHV_QEMU_CPU` can override those values for an explicitly reviewed lab.
...
An extended ordered list can be supplied as comma-separated `AEGISHV_TYPE1_EXPECTED_MARKERS` or `--expect-markers` input. Every custom list must still contain the complete eleven-marker host-table, VMX-backend/VMXON/VMCS-load, owned-paging, and guest-configuration/preemption/I/O/CPUID/HLT/completion chain in order, so configuration cannot weaken the evidence contract to `halt` or another bring-up-only marker. Repeating `--expect-marker` avoids comma parsing when a caller builds the command programmatically:
...
The tests cover VMX feature gates, known-broken preemption-timer CPU-signature refusal, VMXON and VMCS region validation, VMCS lifecycle transitions through VMLAUNCH/VMRESUME, hardware-instruction status decoding, runtime sequencing, true-control and fixed-bit adjustment, complete minimal VMCS construction, guest paging, EPT, preemption-timer and port-I/O qualification handling, CPUID/MSR/CR/HLT exit handling, VPID validation, execute/write trap windows, Monitor Trap Flag fallback behavior, and the fixed toy-guest exit sequence. They do not execute privileged VMX instructions.
