...
They cover memory-map validation, physical page allocation, transactional physical-range reservations, page ownership, huge-page split/merge planning, DMA domains, PCI inventory, crash records, event and command rings, VM lifecycle, vCPU scheduling, early serial logging, x86 page-table plans, ACPI DMAR/IVRS fixture parsing, AP startup plan validation, VMX capability/control validation, VMCS construction, guest paging, EPT, and the toy exit state machine. The Type-1 tests also prove that one allocation ledger excludes the linked kernel/current inherited CR3 root and supplies fifteen distinct VMXON, VMCS, guest/EPT, and interception-bitmap pages without allocator reconstruction. Bitmap tests cover trap-all I/O materialization, the exact one-read `IA32_PAT` MSR allowlist, exact readback, address alignment/uniqueness/bounds, both I/O exits, and the trapped synthetic `IA32_EFER` stage. VMCS tests cover PAT control support, valid PAT bytes, guest/host PAT fields, `CR0.TS=1`, `CR0.EM=0`, `CR4.OSFXSR=1`, and exact `#NM` decoding for the fixed x87 and SIMD stages. The owned-host-paging tests cover relocation, W^X and physical-alias refusal, exact RW/NX leaf mappings for the table-pool pages, null/guard holes, NX/LA57/physical-width failures, and live-table validation modulo architectural accessed/dirty bits. These tests do not execute the bare-metal entry point or privileged VMX instructions.
...
`scripts/build-type1-kernel.sh` builds the `x86_64-unknown-none` kernel ELF and writes `target/type1/aegishv-type1-kernel-build.txt`. It requires the Rust `x86_64-unknown-none` target, builds with static relocation and the kernel code model, and records `bootable_image=false` and `qemu_evidence=false` because the ELF alone is not a Limine boot image or runtime proof. The kernel validates the Limine base revision and bounded HHDM, memory-map, and executable-address response state before using the handoff. It installs and verifies owned GDT/TSS/IDT state, captures CPU/MSR capabilities, validates CR0/CR4 fixed bits and VMX controls, and builds one early allocation ledger. That ledger excludes the linked kernel span and inherited active CR3 root before supplying fifteen distinct VMXON, VMCS, guest/EPT, and interception-bitmap pages from Limine `USABLE` memory below 4 GiB. The I/O A/B pages are trap-all. The MSR page starts deny-all, clears exactly the low-read bit for `IA32_PAT`, and is read back before HHDM removal. After all HHDM writes, the final Intel path enables NXE/WP, installs the four-page owned root, and validates live RX/R/RW mappings plus five guards. VMCS configuration requires both bitmap controls and exact live bitmap-address and PAT-field readback.

On an eligible Intel host VMLAUNCH begins with the zero-value VMX preemption-timer sentinel, then resumes a finite TSC-or-count probe with a reload derived from the hard `0x01000000`-TSC-tick budget and `IA32_VMX_MISC`. Only a real nonzero timer expiration advances RIP to the payload. The fixed payload performs byte writes to ports `0xe9` and `0x8000`, CPUID leaf/subleaf 0, trapped `RDMSR IA32_EFER`, direct `RDMSR IA32_PAT`, `FNOP`, `MOVDQA xmm0,xmm0`, a fixed `UD2` sentinel, and HLT. The guest PAT read must match the deliberate valid VMCS value. The next two stages must exit as exact vector-7 `#NM` faults at their fixed RIPs under `TS=1`, `EM=0`, and `OSFXSR=1`; neither probe is advanced through a reported instruction length. After the SIMD `#NM`, the VMM injects one fixed vector-6 hardware exception at VM entry. The immutable CPL0 IDT gate reaches an integer-only handler, which advances the saved `UD2` RIP by two bytes and returns with `IRETQ` to the fixed HLT. The exit handler suppresses the I/O exits, returns synthetic zero only for `IA32_EFER`, checks live host PAT restoration and the saved guest PAT on every exit, checks the direct-read result at both guard exits, validates the injection cookie, restored guest stack, cleared VM-entry valid bit, and clear IDT-vectoring valid bit at HLT, and keeps every resume bounded before VMXOFF.

`scripts/inspect-type1-kernel.sh` checks the built kernel ELF. When `llvm-readobj` is available it verifies the expected entry address, three page-separated RX/R/RW load segments, section layout, four-page table pool symbols, five 4K guard sections, exact stack sizes, `.limine_requests`, and static/no-relocation shape. Its host `.text` disassembly gate rejects FPU, MMX/SIMD, and FXSAVE/XSAVE-family state-save instructions so the fixed host path does not silently acquire an untracked SIMD dependency. It also checks the host-table, host-paging, backend, runtime preflight/enable/region, VMXON, VMCS-load, guest configuration/exit/completion, missing-Limine, and status-specific failure marker bytes. It writes `target/type1/aegishv-type1-kernel-inspect.txt`. This is artifact inspection, not execution evidence or a replacement for FPU/SIMD context management.
...
`scripts/type1-qemu-smoke.sh` is opt-in lab plumbing for the bootable Limine ISO. It deliberately refuses a raw ELF because QEMU `-kernel` does not provide the required Limine handoff. By default, success requires the complete serial lines `aegishv:type1:host-tables-ok`, `aegishv:type1:backend-vmx`, `aegishv:type1:vmxon-cycle-ok`, `aegishv:type1:vmcs-load-ok`, `aegishv:type1:host-paging-ok`, `aegishv:type1:guest-config-ok`, `aegishv:type1:guest-preempt-exit-ok`, `aegishv:type1:guest-io-exit-ok`, `aegishv:type1:guest-io-b-exit-ok`, `aegishv:type1:guest-cpuid-exit-ok`, `aegishv:type1:guest-rdmsr-exit-ok`, `aegishv:type1:guest-pat-state-ok`, `aegishv:type1:guest-nm-x87-exit-ok`, `aegishv:type1:guest-nm-simd-exit-ok`, `aegishv:type1:guest-ud-inject-ok`, `aegishv:type1:guest-hlt-exit-ok`, and `aegishv:type1:guest-run-ok` in that order. A log is rejected if it also contains a contradictory backend, host-table, host-paging, or runtime failure, skipped VMX operation, `aegishv:type1:guest-timeout`, guest entry/exit/resume error, `aegishv:type1:guest-ud-inject-error`, other unexpected exception, missing-Limine, or panic marker. The script defaults to `q35,accel=kvm` and `host,+vmx`; `AEGISHV_QEMU_MACHINE` and `AEGISHV_QEMU_CPU` can override those values for an explicitly reviewed lab.

Each configured success marker must appear exactly once. A missing or duplicate line fails the smoke even when one ordered copy of the complete chain is present.

`aegishv:type1:guest-preempt-exit-ok` is emitted only after the zero-value sentinel has been followed by a real nonzero deadline exit from the finite TSC-or-count probe. It is not emitted merely because the VMCS timer field was written. If either finite limit reaches the probe's HLT fallback, the run emits `aegishv:type1:guest-timeout` instead.

`aegishv:type1:guest-pat-state-ok` follows the fixed guest's direct `RDMSR IA32_PAT` comparison and requires the deliberate valid guest PAT, saved guest VMCS field, and restored live host PAT to match their expected values. `guest-nm-x87-exit-ok` and `guest-nm-simd-exit-ok` require exact hardware-exception metadata and fixed fault RIPs for `FNOP` and `MOVDQA` self. They prove only those two denied instructions in this immutable guest. `guest-ud-inject-ok` is emitted only at the later fixed HLT exit after the vector-6 VM-entry event has traversed the immutable CPL0 IDT gate, the integer-only handler has changed its cookie and returned with `IRETQ`, the guest stack has been restored, and the VM-entry and IDT-vectoring valid bits are clear. Writing the VM-entry fields alone is not success.

```bash
scripts/type1-qemu-smoke.sh --print-command ./target/type1/aegishv-type1.iso
```

An extended ordered list can be supplied as comma-separated `AEGISHV_TYPE1_EXPECTED_MARKERS` or `--expect-markers` input. Every custom list must still contain the complete seventeen-marker host-table, VMX-backend/VMXON/VMCS-load, owned-paging, and guest-configuration/preemption/I/O-A/I/O-B/CPUID/RDMSR/PAT/x87-`#NM`/SIMD-`#NM`/fixed-`#UD`-injection/HLT/completion chain in order, so configuration cannot weaken the evidence contract to the old sixteen-marker list, `halt`, or another bring-up-only marker. Repeating `--expect-marker` avoids comma parsing when a caller builds the command programmatically:

Duplicate entries in a custom expected-marker list are rejected before QEMU starts.
...
scripts/type1-qemu-smoke.sh \
  ...
  --expect-marker aegishv:type1:host-paging-ok \
  --expect-marker aegishv:type1:guest-config-ok \
  --expect-marker aegishv:type1:guest-preempt-exit-ok \
  --expect-marker aegishv:type1:guest-io-exit-ok \
  --expect-marker aegishv:type1:guest-io-b-exit-ok \
  --expect-marker aegishv:type1:guest-cpuid-exit-ok \
  --expect-marker aegishv:type1:guest-rdmsr-exit-ok \
  --expect-marker aegishv:type1:guest-pat-state-ok \
  --expect-marker aegishv:type1:guest-nm-x87-exit-ok \
  --expect-marker aegishv:type1:guest-nm-simd-exit-ok \
  --expect-marker aegishv:type1:guest-ud-inject-ok \
...
`scripts/type1-qemu-evidence.sh` wraps the QEMU smoke script for local lab runs and writes `target/type1/aegishv-type1-qemu-evidence.txt`. The manifest includes SHA-256 digests of the boot image before and after QEMU, digest validity and equality, QEMU version, machine, CPU, boot mode, rendered command line, serial log path, ordered marker list, per-marker observations, order result, first contradictory marker, smoke exit code, and the serial-reported VMX CPU signature, timer rate, reload, and effective TSC-tick deadline. `qemu_evidence=true` requires both image digests to be valid and equal, a successful smoke, every expected marker exactly once and in order, no contradictory marker, and exactly one well-formed diagnostic set. Missing hash tooling, malformed hash output, or an image changed during the run fails evidence with status 70. The evidence helper also checks that the timer rate fits the architectural field, the reload is at least 2, the effective value equals `reload << rate`, and that value does not exceed the hard `0x01000000`-tick budget. These diagnostics are audited in addition to the seventeen-marker chain; they do not add markers to it. With the default contract, this proves the final-path owned-root and bitmap/PAT-field readbacks plus one VMLAUNCH into the fixed guest, its nonzero preemption timer, contained I/O exits, CPUID, synthetic high-read RDMSR exit, the fixed PAT transition, exact x87 and SIMD `#NM` exits, and one fixed vector-6 hardware exception injected at VM entry through the immutable CPL0 IDT gate before an integer-only handler returns with `IRETQ` to the fixed HLT. It does not prove general PAT policy, FPU/SIMD context management, general exceptions, exceptions with error codes, reinjection, nested delivery-fault recovery, IST or privilege transitions, external interrupts, APIC, SMP, guest-OS support, WRMSR, or production readiness.

The evidence manifest records `serial_markers_exactly_once`; `qemu_evidence=true` requires that field as well as the ordered-chain result.

A local modern-Limine ISO has booted under QEMU TCG through owned descriptor-table installation and runtime preflight. That environment reports no VMX support, and WHPX is unavailable, so it exits before the final-path CR3 switch and does not satisfy the strict evidence contract. This is useful boot-boundary evidence only; it must not be cited as owned-paging or Intel guest-execution evidence.

```bash
scripts/type1-qemu-evidence.sh --image ./target/type1/aegishv-type1.iso --timeout 20
```

`scripts/run-type1-lab.sh` is the one-command local chain for reviewed lab hosts. It refuses to run unless `AEGISHV_RUN_TYPE1_LAB=1` is set, then runs `check-type1-lab-tools.sh --require-all`, builds the Limine ISO, runs QEMU evidence capture, and writes `target/type1/aegishv-type1-lab-summary.txt`. The summary records the expected list, exact-once and order results, contradictory-marker refusal, machine, CPU, boot mode, and rendered QEMU command; `lab_complete=true` requires the strict evidence result. The runner forwards `--expect-markers` and repeated `--expect-marker` arguments when a lab needs an explicit non-default contract.
...
The tests cover VMX feature gates, known-broken preemption-timer CPU-signature refusal, VMXON and VMCS region validation, VMCS lifecycle transitions through VMLAUNCH/VMRESUME, hardware-instruction status decoding, runtime sequencing, true-control and fixed-bit adjustment, bitmap controls/addresses/readback, complete minimal VMCS construction, guest paging, EPT, preemption-timer and both port-I/O qualification paths, the fixed synthetic `IA32_EFER` response, the one-read PAT allowlist, PAT VMCS controls and fields, exact `#NM` interruption metadata, fixed vector-6 VM-entry event encoding/readback, immutable guest GDT/IDT/handler layout, injection-stage and delivery-failure handling, CPUID/MSR/CR/HLT exit handling, VPID validation, execute/write trap windows, Monitor Trap Flag fallback behavior, and the fixed toy-guest exit sequence. They do not execute privileged VMX instructions.
...
- Owned paging across early boot and all CPUs, dynamic mappings/invalidation, physical/MMIO policy, teardown/reclamation, and recoverable guard-fault testing beyond the bounded final Intel path.
- SMP/per-CPU VMX, APIC/interrupt/guest-timer virtualization, vCPU scheduling, and scheduler-driven preemption. The fixed guest's VMX preemption timer is only a stage deadline.
- An independent host watchdog and hardware qualification for timer failures outside the fixed known-broken CPU-signature denylist.
- General guest loading, device backends, and IOMMU-enforced DMA isolation.
- XSAVE/FXSAVE, host FPU/SIMD preservation and context switching, lazy/multi-vCPU FPU state, WRMSR PAT, MTRR/PAT/MMIO policy, SMP/per-CPU PAT, full required MSR context, general exception delivery and reinjection, exceptions with error codes, IST or privilege transitions, external interrupts/APIC, and broad exit coverage beyond the fixed `#NM` and vector-6 probes.
