...
On an eligible Intel host VMLAUNCH begins with the zero-value VMX preemption-timer sentinel, then resumes a finite TSC-or-count probe with a reload derived from the hard `0x01000000`-TSC-tick budget and `IA32_VMX_MISC`. Only a real nonzero timer expiration advances RIP to the payload. The fixed payload performs byte writes to ports `0xe9` and `0x8000`, CPUID leaf/subleaf 0, trapped `RDMSR IA32_EFER`, direct `RDMSR IA32_PAT`, `FNOP`, `MOVDQA xmm0,xmm0`, and HLT. The guest PAT read must match the deliberate valid VMCS value. The next two stages must exit as exact vector-7 `#NM` faults at their fixed RIPs under `TS=1`, `EM=0`, and `OSFXSR=1`; neither probe is advanced through a reported instruction length. The handler suppresses the I/O exits, returns synthetic zero only for `IA32_EFER`, checks live host PAT restoration and the saved guest PAT on every exit, checks the direct-read result at both guard exits, and keeps every resume bounded before HLT and VMXOFF.
...
`scripts/type1-qemu-smoke.sh` is opt-in lab plumbing for the bootable Limine ISO. It deliberately refuses a raw ELF because QEMU `-kernel` does not provide the required Limine handoff. By default, success requires the complete serial lines `aegishv:type1:host-tables-ok`, `aegishv:type1:backend-vmx`, `aegishv:type1:vmxon-cycle-ok`, `aegishv:type1:vmcs-load-ok`, `aegishv:type1:host-paging-ok`, `aegishv:type1:guest-config-ok`, `aegishv:type1:guest-preempt-exit-ok`, `aegishv:type1:guest-io-exit-ok`, `aegishv:type1:guest-io-b-exit-ok`, `aegishv:type1:guest-cpuid-exit-ok`, `aegishv:type1:guest-rdmsr-exit-ok`, `aegishv:type1:guest-pat-state-ok`, `aegishv:type1:guest-nm-x87-exit-ok`, `aegishv:type1:guest-nm-simd-exit-ok`, `aegishv:type1:guest-hlt-exit-ok`, and `aegishv:type1:guest-run-ok` in that order. A log is rejected if it also contains a contradictory backend, host-table, host-paging, or runtime failure, skipped VMX operation, `aegishv:type1:guest-timeout`, guest entry/exit/resume error, exception, missing-Limine, or panic marker. The script defaults to `q35,accel=kvm` and `host,+vmx`; `AEGISHV_QEMU_MACHINE` and `AEGISHV_QEMU_CPU` can override those values for an explicitly reviewed lab.
...
An extended ordered list can be supplied as comma-separated `AEGISHV_TYPE1_EXPECTED_MARKERS` or `--expect-markers` input. Every custom list must still contain the complete sixteen-marker host-table, VMX-backend/VMXON/VMCS-load, owned-paging, and guest-configuration/preemption/I/O-A/I/O-B/CPUID/RDMSR/PAT/x87-`#NM`/SIMD-`#NM`/HLT/completion chain in order, so configuration cannot weaken the evidence contract to `halt` or another bring-up-only marker. Repeating `--expect-marker` avoids comma parsing when a caller builds the command programmatically:
...
- XSAVE/FXSAVE, host FPU/SIMD preservation and context switching, lazy/multi-vCPU FPU state, WRMSR PAT, MTRR/PAT/MMIO policy, SMP/per-CPU PAT, full required MSR context, general exception injection, and broad exit coverage beyond the two fixed `#NM` probes.
