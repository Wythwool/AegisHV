...
They cover memory-map validation, physical page allocation, transactional physical-range reservations, page ownership, huge-page split/merge planning, DMA domains, PCI inventory, crash records, event and command rings, VM lifecycle, vCPU scheduling, early serial logging, x86 page-table plans, ACPI DMAR/IVRS fixture parsing, AP startup plan validation, VMX capability/control validation, VMCS construction, guest paging, EPT, and the toy exit state machine. The Type-1 tests also prove that one allocation ledger excludes the linked kernel/current inherited CR3 root and supplies VMXON, VMCS, and all guest/EPT pages without allocator reconstruction. These tests do not execute the bare-metal entry point or privileged VMX instructions.
...
`scripts/type1-qemu-smoke.sh` is opt-in lab plumbing for the bootable Limine ISO. It deliberately refuses a raw ELF because QEMU `-kernel` does not provide the required Limine handoff. By default, success requires the complete serial lines `aegishv:type1:host-tables-ok`, `aegishv:type1:backend-vmx`, `aegishv:type1:vmxon-cycle-ok`, `aegishv:type1:vmcs-load-ok`, `aegishv:type1:guest-config-ok`, `aegishv:type1:guest-preempt-exit-ok`, `aegishv:type1:guest-io-exit-ok`, `aegishv:type1:guest-cpuid-exit-ok`, `aegishv:type1:guest-hlt-exit-ok`, and `aegishv:type1:guest-run-ok` in that order. A log is rejected if it also contains a contradictory backend, host-table or runtime failure, skipped VMX operation, `aegishv:type1:guest-timeout`, guest entry/exit/resume error, exception, missing-Limine, or panic marker. The script defaults to `q35,accel=kvm` and `host,+vmx`; `AEGISHV_QEMU_MACHINE` and `AEGISHV_QEMU_CPU` can override those values for an explicitly reviewed lab.
