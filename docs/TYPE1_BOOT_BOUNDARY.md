...
- `crates/aegishv-type1-kernel` builds a minimal `x86_64-unknown-none` kernel ELF that carries the first Limine request block, writes the planned success marker only after the minimal Limine handoff has accepted base revision, response revisions, HHDM offset, nonempty memory-map with entries pointer, and executable-address responses, writes specific fallback markers for incomplete handoffs, and halts when its entry path is reached.
