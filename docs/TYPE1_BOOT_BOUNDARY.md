...
- The type-1 kernel can choose the checked VMX/SVM runtime plan from a CPU capability snapshot, apply the CR0/CR4/EFER values needed before entry, materialize the selected runtime pages, and run VMXON followed by VMXOFF for the Intel VMX backend, but it does not load a VMCS, launch or resume a guest, or execute VMRUN; EL2 entry is not implemented by this milestone.
