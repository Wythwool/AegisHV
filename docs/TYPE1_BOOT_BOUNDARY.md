...
- `crates/aegishv-type1-kernel` builds a minimal `x86_64-unknown-none` kernel ELF that carries the first Limine request block, writes the planned success marker only after the minimal Limine handoff has accepted base revision, response revisions, HHDM offset, nonempty memory-map with entries pointer, and executable-address bases matching the linker layout, reads a bounded CPUID/MSR snapshot for VMX/SVM capability selection, writes a runtime backend marker for the checked VMX/SVM planning boundary, writes specific fallback markers for incomplete handoffs, and halts when its entry path is reached.
- `scripts/build-type1-kernel.sh` writes `target/type1/aegishv-type1.elf` and a kernel build manifest.
- `scripts/inspect-type1-kernel.sh` records local ELF inspection for the expected entry address, section layout, boot stack size, success marker bytes, runtime backend marker bytes, CPUID/MSR probe marker coverage, missing-handoff marker bytes, and status-specific handoff marker bytes.
...
- The type-1 kernel can choose the checked VMX/SVM runtime plan from a CPU capability snapshot, but it does not call the VMXON/VMLAUNCH/VMRESUME or EFER.SVME/VMRUN execution paths yet; EL2 entry is not implemented by this milestone.
