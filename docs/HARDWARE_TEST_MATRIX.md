...
| Intel VMX toy-guest runtime | implemented | One ledger owns 15 distinct pages including trap-all I/O A/B and a one-read-allowlisted MSR page; the VMCS requires exact bitmap and PAT-field readback; bounded I/O, CPUID, synthetic `IA32_EFER`, deliberate PAT, exact x87/SIMD `#NM`, one fixed vector-6 VM-entry injection through an immutable CPL0 IDT gate, and HLT stages are wired in code | Matching valid pre/post-run SHA-256 image digests, complete seventeen-marker evidence, and validated CPU/timer diagnostics required |
...
| Area | Status | Current Evidence | Release Gate |
| --- | --- | --- | --- |
| Fixed Intel deadline/I/O/`CPUID`/MSR/PAT/`#NM`/`#UD`-injection/`HLT` toy guest | implemented | Guest bytes, finite timeout fallback, four-level paging/EPT, two suppressed port exits, trapped synthetic `IA32_EFER`, direct `IA32_PAT` readback, exact `#NM` stages for `FNOP` and `MOVDQA` self, and one fixed CPL0 vector-6 injection/`IRETQ` round trip build and test | Seventeen-marker hardware chain required before execution claim; not general exception or interrupt support |
...
The following coverage is still missing: owned paging throughout early boot, dynamic/per-CPU roots and invalidation, teardown/recovery, executed guard-fault evidence, SMP/per-CPU VMX, APIC/interrupt/guest-timer virtualization, devices/IOMMU isolation, and a general guest loader. The narrow fixed-guest proof does not provide XSAVE/FXSAVE, host SIMD preservation or context switching, lazy/multi-vCPU FPU state, WRMSR PAT, MTRR/PAT/MMIO policy, SMP/per-CPU PAT, full stateful MSR handling, general exception delivery, exceptions with error codes, reinjection, nested delivery-fault recovery, IST or privilege transitions, external interrupts/APIC, SMP, or guest-OS exception support. Selective/dynamic bitmap policy, live AMD/ARM paths, hardware soak, and the secure update/attestation/incident-response lifecycle also remain release blockers for production wording.
