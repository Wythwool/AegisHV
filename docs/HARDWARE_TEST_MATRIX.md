...
| Intel VMX toy-guest runtime | implemented | One ledger owns 15 distinct pages including trap-all I/O A/B and a one-read-allowlisted MSR page; the VMCS requires exact bitmap and PAT-field readback; bounded I/O, CPUID, synthetic `IA32_EFER`, deliberate PAT, exact x87/SIMD `#NM`, and HLT stages are wired in code | Matching valid pre/post-run SHA-256 image digests, complete sixteen-marker evidence, and validated CPU/timer diagnostics required |
...
| Area | Status | Current Evidence | Release Gate |
| --- | --- | --- | --- |
| Fixed Intel deadline/I/O/`CPUID`/MSR/PAT/`#NM`/`HLT` toy guest | implemented | Guest bytes, finite timeout fallback, four-level paging/EPT, two suppressed port exits, trapped synthetic `IA32_EFER`, direct `IA32_PAT` readback, and exact `#NM` stages for `FNOP` and `MOVDQA` self build and test | Sixteen-marker hardware chain required before execution claim |
