...
The live path remains BSP-only and lacks interrupt injection, APIC and guest-timer virtualization, scheduler-driven preemption, an independent host watchdog, devices, XSAVE state, general guest loading, and hardware qualification. The fixed guest's VMX preemption timer and in-guest fallback are containment checks, not those missing runtime facilities. The signature denylist does not cover unknown timer or TSC errata. A source build or model test is not VMX execution evidence; use the strict full marker chain and CPU/timer diagnostic audit described in `TYPE1_BOOT_BOUNDARY.md` on a reviewed nested-VMX or bare-metal host.
