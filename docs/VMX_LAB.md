...
These pieces are library code with tests. Normal tests do not execute privileged VMX instructions. The type-1 kernel can now select a checked VMX runtime plan and apply the CR0/CR4 enable values before entry, but VMXON/VMLAUNCH/VMRESUME execution is not wired into the boot image or hardware evidence path yet.
