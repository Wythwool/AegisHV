...
- One retained allocator ledger for fifteen distinct VMXON/VMCS/guest/EPT/bitmap pages, trap-all bitmap materialization, both bitmap controls, strict bitmap-address validation, and exact live VMCS address readback.
...
- Controlled guest entry, a zero-value VMX preemption-timer sentinel followed by a proven nonzero deadline exit from a finite TSC-or-count probe with an HLT timeout fallback, validated and suppressed exits through I/O bitmap A and B, CPUID, a synthetic high-read RDMSR result, and HLT separated by bounded VMRESUME operations, followed by VMXOFF.
...
- Complete architectural context policy, including PAT, XSAVE/FPU state, full required MSR state, WRMSR semantics, selective and dynamic I/O/MSR bitmap policy, interrupt injection, and broader exit handling.
...
The Intel toy-guest path is also present in runtime code. One early allocation ledger excludes the complete linked kernel image and inherited active CR3 root before allocating fifteen distinct Limine `USABLE` pages below 4 GiB: VMXON, VMCS, ten guest/EPT pages, and trap-all I/O A, I/O B, and MSR pages. Invalid bounds, allocation aliases, or a changed inherited root fail closed. The bitmap pages are filled and read back before the HHDM is removed. The VMCS requires both bitmap controls, validates nonzero/aligned/distinct/below-4-GiB addresses, and requires exact live address readback. The final path then enables NXE/WP, rejects LA57, switches to the four-page owned hierarchy, and validates the live RX/R/RW leaves and five guards before guest entry.

The fixed guest enters a finite TSC-or-count deadline probe with an HLT fallback. A zero-value sentinel precedes a nonzero timer deadline derived from the hard `0x01000000`-TSC-tick budget; reloads below 2 are refused. After that deadline the guest executes contained byte writes to ports `0xe9` and `0x8000`, CPUID leaf/subleaf 0, `RDMSR IA32_EFER`, and HLT. The handler validates both bitmap-governed I/O exits, returns synthetic zero for the high-read MSR exit, and never executes host I/O or host MSR on the guest's behalf. Every resume remains bounded.

The repository still does not pass the Intel guest-entry evidence gate. The available TCG environment does not expose VMX, and WHPX is unavailable, so the local boot reaches the non-VMX/skip path before the owned-root switch rather than the required digest-stable thirteen-marker guest evidence package. There is no reviewed nested-VMX or bare-metal log proving that the wired guest, bitmap exits, RDMSR handling, or final-path CR3 executed.
