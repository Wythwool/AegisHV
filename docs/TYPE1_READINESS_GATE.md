...
- Controlled guest entry, a zero-value VMX preemption-timer sentinel followed by a proven nonzero deadline exit from a finite TSC-or-count probe with an HLT timeout fallback, validated and suppressed exits through I/O bitmap A and B, CPUID, a synthetic high-read RDMSR result, deliberate guest PAT load/read/save plus host restore, exact `#NM` exits for fixed `FNOP` and `MOVDQA`-self probes, one fixed vector-6 VM-entry injection through an immutable CPL0 IDT gate and integer-only `IRETQ` handler, and HLT separated by bounded VMRESUME operations, followed by VMXOFF.
...
The fixed guest enters a finite TSC-or-count deadline probe with an HLT fallback. A zero-value sentinel precedes a nonzero timer deadline derived from the hard `0x01000000`-TSC-tick budget; reloads below 2 are refused. After that deadline the guest executes the two contained port operations, CPUID, trapped `RDMSR IA32_EFER`, direct `RDMSR IA32_PAT`, `FNOP`, `MOVDQA xmm0,xmm0`, a fixed `UD2` sentinel, and HLT. The deliberate valid guest PAT is loaded on entry, saved on exit, compared by the guest, and separated from the restored host PAT. The two probe instructions must produce exact `#NM` exits under `TS=1`, `EM=0`, and `OSFXSR=1`. After the SIMD exit the state machine injects one vector-6 hardware exception through the immutable CPL0 IDT gate; its integer-only handler advances the saved `UD2` RIP and returns with `IRETQ` to HLT. The host `.text` disassembly gate rejects FPU/SIMD/state-save instructions, but no general FPU/SIMD, exception, or interrupt management is claimed.

The repository still does not pass the Intel guest-entry evidence gate. The available TCG environment does not expose VMX, and WHPX is unavailable, so the local boot reaches the non-VMX/skip path before the owned-root switch rather than the required digest-stable seventeen-marker guest evidence package. There is no reviewed nested-VMX or bare-metal log proving that the wired guest, bitmap exits, PAT transition, `#NM` guards, fixed vector-6 injection/`IRETQ` round trip, or final-path CR3 executed.
