...
- A fixed guest with a finite deadline probe followed by the two contained port writes, CPUID, trapped synthetic `IA32_EFER`, direct `IA32_PAT`, `FNOP`, `MOVDQA` self, a fixed `UD2` sentinel, and HLT. The VMCS loads a deliberate valid guest PAT, saves it on exit, restores the host PAT, and checks both. Under `TS=1`, `EM=0`, and `OSFXSR=1`, the x87 and SIMD probes must each produce an exact `#NM` at its fixed RIP. After the SIMD exit the state machine injects one vector-6 hardware exception through an immutable CPL0 IDT gate; an integer-only handler returns with `IRETQ` to the fixed HLT. The host `.text` disassembly gate rejects FPU/SIMD/state-save instructions. These are fixed boundary checks, not general context, exception, or interrupt virtualization.
- A strict evidence contract that requires matching valid pre/post-run SHA-256 image digests, a seventeen-marker chain including owned-host-paging validation, preemption, I/O-A, I/O-B, CPUID, trapped RDMSR, PAT state, x87 `#NM`, SIMD `#NM`, fixed `#UD` injection, and HLT exits, plus one internally consistent CPU-signature/timer diagnostic set. It rejects changed images, duplicate or out-of-order markers, the old sixteen-marker contract, contradictory backends, paging failures, skipped VMX operations, host faults, guest timeouts, injection failures, unexpected exceptions, and guest entry/exit/resume failures.
...
Intel guest execution remains unproven until a reviewed nested-VMX or bare-metal run captures matching valid pre/post-run SHA-256 image digests, the complete marker chain, and validated CPU/timer diagnostics described in `TYPE1_BOOT_BOUNDARY.md`. Even successful evidence proves only one BSP, one fixed guest, two trap-all I/O exits, one synthetic RDMSR result, one deliberate PAT transition, two exact fixed-probe `#NM` exits, and one fixed vector-6 VM-entry injection through the immutable CPL0 IDT gate before an integer-only handler returns with `IRETQ` to the fixed HLT. It does not prove general PAT policy, XSAVE/FXSAVE, host FPU/SIMD preservation or context switching, lazy/multi-vCPU state, WRMSR, general exceptions, error-code injection, reinjection, IST or privilege transitions, external interrupts/APIC, SMP, guest-OS support, or production qualification.
...
- Full architectural context policy beyond the fixed proof: XSAVE/FXSAVE, host FPU/SIMD preservation and context switching, lazy/multi-vCPU state, WRMSR PAT, MTRR/PAT/MMIO policy, SMP/per-CPU PAT, required stateful MSRs, selective/dynamic I/O and MSR bitmaps, general exception delivery and reinjection, exceptions with error codes, nested delivery-fault recovery, IST and privilege transitions, external interrupts, and broad exit coverage. The fixed bitmap, PAT/`#NM`, and vector-6 stages are not a general device, exception, or architectural-state policy.
...
4. Extend the fixed PAT/`#NM`/vector-6 proof into architectural context handling: XSAVE/FXSAVE, host and guest FPU/SIMD preservation, lazy/multi-vCPU state, per-CPU PAT plus MTRR/MMIO policy, stateful MSRs and WRMSR, selective/dynamic bitmaps, general exception delivery and reinjection, error-code and delivery-fault semantics, and recovery paths.
