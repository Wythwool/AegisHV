...
- Guest page materialization zeros a full page before writing its bounded contents. Each interception page is instead filled with `0xff` and read back exactly so every covered I/O or MSR access traps by default.
...
- The trap-all MSR page contains the high-read bit for `IA32_EFER`. The only accepted RDMSR exit occurs in its exact stage and returns synthetic zero in EDX:EAX without executing host RDMSR. General MSR virtualization and WRMSR are not implemented.
- The only accepted successful exit order is the zero-value preemption-timer sentinel, the nonzero timer expiry from the finite probe, I/O bitmap A, I/O bitmap B, CPUID, RDMSR, and HLT. An HLT or timer exit at the exact probe fallback RIP is classified as `guest-timeout`; a timer expiry during a later payload stage, any other unexpected exit, or any out-of-order exit is terminal. Unexpected non-fallback probe exits remain guest-exit errors.
...
- Intel guest evidence requires the complete ordered host-table, VMX backend, VMXON, VMCS-load, owned-host-paging, guest-configuration, preemption-exit, I/O-A-exit, I/O-B-exit, CPUID-exit, RDMSR-exit, HLT-exit, and guest-run marker chain.
- Evidence also requires exactly one well-formed CPU-signature and timer-diagnostic set. The timer rate, reload, and effective deadline must be internally consistent, the reload must be at least 2, and the effective value must not exceed the hard `0x01000000`-TSC-tick budget. These diagnostics do not change the thirteen-marker count.
...
- Even a digest-stable valid thirteen-marker run proves only the fixed toy guest, its trap-all bitmap exits, and the final-path owned root on the recorded CPU/firmware/accelerator configuration. It does not prove selective device access, general MSR virtualization, or WRMSR, and it does not prove a general or production hypervisor.
