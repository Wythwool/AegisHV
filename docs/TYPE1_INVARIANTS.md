...
- After the SIMD `#NM`, exactly one vector-6 hardware exception is programmed for VM entry at the fixed `UD2` RIP. Its immutable CPL0 IDT gate and integer-only handler must return with `IRETQ` to the fixed HLT with the expected cookie, original stack, cleared VM-entry valid bit, and clear IDT-vectoring valid bit. Delivery-time faults and EPT exits are terminal; this fixed path performs no reinjection.
- The only accepted successful exit order is the zero-value preemption-timer sentinel, nonzero timer expiry, I/O bitmap A, I/O bitmap B, CPUID, trapped EFER read, x87 `#NM`, SIMD `#NM`, and HLT; the direct PAT read executes between the EFER and x87 exits, and the fixed vector-6 delivery occurs between the SIMD exit and HLT. An HLT or timer exit at the probe fallback RIP is classified as `guest-timeout`; a later timer expiry, wrong exception, delivery-time fault, unexpected exit, or out-of-order exit is terminal.
...
- Intel guest evidence requires the complete ordered host-table, VMX backend, VMXON, VMCS-load, owned-host-paging, guest-configuration, preemption-exit, I/O-A-exit, I/O-B-exit, CPUID-exit, RDMSR-exit, PAT-state, x87-`#NM`, SIMD-`#NM`, fixed-`#UD`-injection, HLT-exit, and guest-run marker chain.
- Evidence also requires exactly one well-formed CPU-signature and timer-diagnostic set. The timer rate, reload, and effective deadline must be internally consistent, the reload must be at least 2, and the effective value must not exceed the hard `0x01000000`-TSC-tick budget. These diagnostics do not change the seventeen-marker count.
...
- Even a digest-stable valid seventeen-marker run proves only the fixed toy guest, its two trap-all I/O exits, one-read MSR allowlist, deliberate PAT transition, two fixed `#NM` probes, one fixed vector-6 VM-entry injection/`IRETQ` round trip, and final-path owned root on the recorded CPU/firmware/accelerator configuration. It does not prove general PAT policy, FPU/SIMD virtualization or context switching, selective device access, general MSR/exception virtualization, error-code injection, reinjection, IST or privilege transitions, external interrupts/APIC, SMP, guest-OS support, or WRMSR, and it does not prove a general or production hypervisor.
