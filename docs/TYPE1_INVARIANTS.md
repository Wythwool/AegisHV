...
- Intel guest evidence requires the complete ordered host-table, VMX backend, VMXON, VMCS-load, owned-host-paging, guest-configuration, preemption-exit, I/O-exit, CPUID-exit, HLT-exit, and guest-run marker chain.
- Evidence also requires exactly one well-formed CPU-signature and timer-diagnostic set. The timer rate, reload, and effective deadline must be internally consistent, the reload must be at least 2, and the effective value must not exceed the hard `0x01000000`-TSC-tick budget. These diagnostics do not change the eleven-marker count.
...
- Even a digest-stable valid eleven-marker run proves only the fixed toy guest and final-path owned root on the recorded CPU/firmware/accelerator configuration. It does not prove a general or production hypervisor.
