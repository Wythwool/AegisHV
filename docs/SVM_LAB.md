...
These pieces are library code with tests. Normal tests do not execute privileged SVM instructions. The type-1 kernel can now select a checked SVM runtime plan, apply the EFER.SVME enable value, and materialize the VMCB page before entry, but VMRUN execution is not wired into the boot image or hardware evidence path yet.
