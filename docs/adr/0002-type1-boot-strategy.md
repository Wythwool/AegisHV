...
The Intel path nevertheless wires a finite TSC-or-count deadline probe with an HLT fallback and a fixed `AL='A'; OUT 0xE9,AL; CPUID leaf/subleaf 0; HLT` payload, guest paging, EPT, complete VMCS setup, and an assembly entry/exit trampoline. A zero-value VMX preemption-timer sentinel is followed by a real nonzero deadline exit from the probe before the host moves RIP to the payload. If the timer fails, the probe reaches its HLT fallback through either the TSC horizon or the finite iteration count, reports `guest-timeout`, and regains control. Unconditional I/O exiting, validated and suppressed port-I/O handling, bounded resumes through CPUID and HLT, and VMX shutdown follow. It requires a reviewed nested-VMX or bare-metal run with matching valid pre/post-run SHA-256 image digests, the complete strict marker chain, and validated CPU/timer diagnostics before guest execution is claimed.
...
Workspace tests include the `no_std` crates and bare-metal kernel build checks. QEMU boot remains opt-in and is not part of normal CI. The evidence path requires matching valid pre/post-run SHA-256 image digests and refuses to treat the run as Intel evidence unless the full host-table, VMX-backend/VMXON/VMCS-load, owned-paging, guest-configuration/preemption/I/O/CPUID/HLT/completion chain appears in order without a forbidden marker.
