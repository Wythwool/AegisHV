...
The Intel path nevertheless wires a finite TSC-or-count deadline probe with an HLT fallback and a fixed payload containing byte writes to ports `0xe9` and `0x8000`, CPUID, trapped `RDMSR IA32_EFER`, direct `RDMSR IA32_PAT`, `FNOP`, `MOVDQA` self, a fixed `UD2` sentinel, and HLT, plus guest paging, EPT, complete VMCS setup, and an assembly entry/exit trampoline. One allocator owns fifteen distinct pages, including trap-all I/O A/B pages and one fixed MSR page that permits exactly the PAT read. The VMCS loads a deliberate valid guest PAT, saves it on exit, restores the host PAT, and checks both values. The two fixed probes must exit as exact `#NM` under `TS=1`, `EM=0`, and `OSFXSR=1`; after the SIMD exit the fixed state machine injects one vector-6 hardware exception through an immutable CPL0 IDT gate and an integer-only handler returns with `IRETQ` to HLT. Host `.text` is separately disassembly-gated against FPU/SIMD/state-save instructions. These are narrow fixed-path invariants, not general PAT, FPU/SIMD, exception, or interrupt management. A reviewed nested-VMX or bare-metal run with matching valid pre/post-run SHA-256 image digests, the complete strict marker chain, and validated CPU/timer diagnostics is still required before guest execution is claimed.
...
Workspace tests include the `no_std` crates and bare-metal kernel build checks. QEMU boot remains opt-in and is not part of normal CI. The evidence path requires matching valid pre/post-run SHA-256 image digests and refuses to treat the run as Intel evidence unless the full seventeen-marker host-table, VMX-backend/VMXON/VMCS-load, owned-paging, guest-configuration/preemption/I/O-A/I/O-B/CPUID/RDMSR/PAT/x87-`#NM`/SIMD-`#NM`/fixed-`#UD`-injection/HLT/completion chain appears exactly once and in order without a forbidden marker.
