...
- The Intel VMX instruction/runtime layer is wired into the type-1 kernel as a checked runtime plan only after a CPUID/MSR capability snapshot, CR0/CR4 fixed-bit preflight, controlled CR0/CR4 enable writes, HHDM materialization of the VMXON and VMCS pages, and a VMXON/VMXOFF smoke cycle; the kernel entry path does not load a VMCS, execute VMLAUNCH/VMRESUME, or claim QEMU/hardware evidence.
