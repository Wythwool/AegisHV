...
- Planned type-1 boot skeleton artifacts: a no-std boot handoff crate, Limine config, x86_64 linker script, x86_64 entry symbol stub, and build-plan helper. These artifacts do not produce a bootable image.
...
- Bootable type-1 image, APIC startup, real trampoline code, and QEMU boot evidence are not implemented. The Limine config and linker script are skeleton artifacts only.
