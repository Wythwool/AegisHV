...
- A wired Intel toy guest with a finite TSC-or-count deadline probe and HLT fallback followed by byte `OUT` operations to ports `0xe9` and `0x8000`, CPUID leaf/subleaf 0, `RDMSR IA32_EFER`, and HLT. One allocation ledger reserves the linked kernel image and inherited active CR3 root, then allocates fifteen distinct `USABLE` pages below 4 GiB: VMXON, VMCS, ten guest/EPT pages, and trap-all I/O A, I/O B, and MSR pages. Bitmap materialization is read back exactly before HHDM removal. The VMCS requires both bitmap controls, rejects zero, unaligned, duplicate, or at/above-4-GiB bitmap addresses, and requires exact live address readback before `guest-config-ok`. After HHDM materialization the path installs the bounded owned root and captures it as VMCS host CR3. The runtime refuses CPU signatures known by Linux KVM to have broken VMX preemption timers. An initial zero-value sentinel precedes the nonzero bounded probe deadline; the finite HLT fallback still reports `guest-timeout` rather than wedging the BSP. Later stages remain bounded. The handler validates both port exits, CPUID, and the high-read MSR exit, returns synthetic zero for RDMSR, and never performs guest I/O or MSR operations on the host before HLT and VMXOFF. Strict evidence requires matching valid pre/post-run SHA-256 image digests, the complete ordered thirteen-marker host-table, VMX, owned-paging, configuration, preemption, I/O-A, I/O-B, CPUID, RDMSR, HLT, and completion chain, plus one internally consistent CPU-signature/timer diagnostic set.
...
- Reviewed Intel guest-execution evidence is not present. The available QEMU TCG environment boots the Limine/descriptor-table path but does not expose VMX, and WHPX is unavailable, so it cannot prove the final owned-CR3 switch, VMXON, VMLAUNCH, the preemption, I/O-A, I/O-B, CPUID, RDMSR, VMRESUME, or HLT exits.
...
- PAT, XSAVE/FPU, comprehensive and stateful MSR context, WRMSR handling, selective/dynamic I/O and MSR bitmap policy, broad exit coverage, and hostile-guest recovery are not implemented. The three fixed trap-all pages and synthetic zero RDMSR response are containment evidence, not a general device or MSR policy.
