...
- APIC startup, real trampoline code, and QEMU boot evidence are not implemented. The kernel ELF, Limine config, linker script, ISO-root staging directory, tool-gated ISO builder, image-plan manifest, and serial-marker contract are boundary artifacts only.
