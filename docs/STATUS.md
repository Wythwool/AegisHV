...
- Planned type-1 boot skeleton artifacts: a no-std boot handoff crate, minimal x86_64 kernel ELF crate, Limine config, x86_64 linker script, x86_64 entry symbol stub, image-plan helper, QEMU serial-marker contract, kernel-build helper, ELF inspection helper, ISO-root staging helper, and build-plan helper. These artifacts do not produce a bootable image.
...
- Bootable type-1 ISO, APIC startup, real trampoline code, and QEMU boot evidence are not implemented. The kernel ELF, Limine config, linker script, ISO-root staging directory, image-plan manifest, and serial-marker contract are boundary artifacts only.
