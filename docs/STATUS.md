...
- The Intel VMX instruction/runtime layer is wired into the type-1 kernel as a checked runtime plan only after a CPUID/MSR capability snapshot, CR0/CR4 fixed-bit preflight, controlled CR0/CR4 enable writes, and HHDM materialization of the VMXON and VMCS pages; the kernel entry path does not execute VMXON/VMLAUNCH/VMRESUME or claim QEMU/hardware evidence.
