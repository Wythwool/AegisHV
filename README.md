...
- Wired Intel VMX toy-guest path with VMXON, a complete VMCS, four-level guest paging and EPT, and one allocator ledger for fifteen distinct pages. Two pages are trap-all I/O A/B bitmaps; the fixed MSR bitmap allows exactly one direct guest read of `IA32_PAT`, while every MSR write and every other read remains trapped. The fixed payload performs the two contained port writes, CPUID, a trapped `RDMSR IA32_EFER` answered with synthetic zero, a direct PAT readback, and side-effect-minimized `FNOP` and `MOVDQA`-self probes that must exit as exact `#NM`. After the SIMD exit, the fixed exit state machine injects one vector-6 hardware exception at VM entry; an immutable CPL0 IDT gate enters an integer-only handler that advances the fixed `UD2` frame and returns with `IRETQ` to the fixed HLT. The VMCS loads a deliberate valid guest PAT on entry, saves it on exit, restores the captured host PAT, and checks both values. These are fixed-guest checks, not general PAT, FPU/SIMD, exception, or interrupt virtualization.
...
The VMX guest path is present in code, but execution is claimed only after matching valid pre/post-run SHA-256 image digests, the strict seventeen-marker chain, and a validated CPU/timer diagnostic set are captured on a reviewed nested-VMX or bare-metal host as described in `docs/TYPE1_BOOT_BOUNDARY.md`. Successful evidence would prove one BSP, the final-path owned CR3 readback, one fixed guest, a nonzero VMX preemption deadline exit, trapped accesses through both I/O bitmap pages, one contained high-range RDMSR exit, one direct guest PAT read matching the deliberate VMCS value, host/guest PAT save-and-restore checks, exact `#NM` exits for the fixed x87 and SIMD probes, and one fixed vector-6 VM-entry injection through the immutable CPL0 IDT gate whose integer-only handler returns with `IRETQ` to the fixed HLT. The host does not emulate or replay the guest's I/O or MSR requests; its own `IA32_PAT` reads verify restoration after VM exit. This remains lab evidence rather than production readiness.

Production blockers still include an owned paging policy for the complete boot lifetime and physical address space, dynamic map/unmap with TLB invalidation, recoverable guard-fault testing, SMP/per-CPU roots and VMX state, APIC/interrupt/guest-timer virtualization and scheduler-driven preemption, an independent host watchdog, and a general guest loader. Architectural-state blockers include XSAVE/FXSAVE, host FPU/SIMD preservation and context switching, lazy and multi-vCPU state, WRMSR PAT, MTRR/PAT/MMIO policy, SMP/per-CPU PAT, complete stateful MSR handling, and general exception delivery and reinjection. The fixed vector-6 proof does not cover exceptions with error codes, nested delivery faults, IST or privilege transitions, external interrupts, APIC, SMP, or guest-OS exception support. Devices and IOMMU isolation, live AMD/ARM backends, hardware soak, and a secure update/attestation/incident-response lifecycle also remain open. The fixed CPU-signature denylist and toy probes cannot rule out unknown timer, TSC, PAT, or FPU/SIMD errata.
