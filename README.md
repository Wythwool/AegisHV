...
- Wired Intel VMX toy-guest path with VMXON, a complete VMCS, four-level guest paging and EPT, and one allocator ledger for fifteen distinct pages. Three of those pages hold trap-all I/O A, I/O B, and MSR bitmaps; the VMCS requires both bitmap controls and exact live address readback. The fixed guest performs contained writes to ports `0xe9` and `0x8000`, CPUID, a trapped `RDMSR IA32_EFER` answered with synthetic zero, and HLT after a bounded preemption probe. No guest I/O or MSR access is replayed on the host.
...
The VMX guest path is present in code, but execution is claimed only after matching valid pre/post-run SHA-256 image digests, the strict thirteen-marker chain, and a validated CPU/timer diagnostic set are captured on a reviewed nested-VMX or bare-metal host as described in `docs/TYPE1_BOOT_BOUNDARY.md`. Successful evidence would prove one BSP, the final-path owned CR3 readback, one fixed guest, a nonzero VMX preemption deadline exit, trapped accesses through both I/O bitmap pages, one contained high-range RDMSR exit, one CPUID exit, bounded resumes, and one HLT exit. The host performs none of the guest's I/O or MSR operations, and this remains lab evidence rather than production readiness.

Production blockers still include an owned paging policy for the complete boot lifetime and physical address space, dynamic map/unmap with TLB invalidation, recoverable guard-fault testing, SMP/per-CPU roots and VMX state, APIC/interrupt/guest-timer virtualization and scheduler-driven preemption, an independent host watchdog, a general guest loader, complete PAT/XSAVE/FPU/MSR context, stateful WRMSR and selective/dynamic bitmap policy, devices and IOMMU isolation, live AMD/ARM backends, hardware soak, and a secure update/attestation/incident-response lifecycle. The fixed CPU-signature denylist and toy probe cannot rule out unknown timer or TSC errata.
