...
The VMX guest path is present in code, but execution is claimed only after matching valid pre/post-run SHA-256 image digests, the strict sixteen-marker chain, and a validated CPU/timer diagnostic set are captured on a reviewed nested-VMX or bare-metal host as described in `docs/TYPE1_BOOT_BOUNDARY.md`. Successful evidence would prove one BSP, the final-path owned CR3 readback, one fixed guest, a nonzero VMX preemption deadline exit, trapped accesses through both I/O bitmap pages, one contained high-range RDMSR exit, one direct guest PAT read matching the deliberate VMCS value, host/guest PAT save-and-restore checks, exact `#NM` exits for the fixed x87 and SIMD probes, bounded resumes, and one HLT exit. The host does not emulate or replay the guest's I/O or MSR requests; its own `IA32_PAT` reads verify restoration after VM exit. This remains lab evidence rather than production readiness.

Production blockers still include an owned paging policy for the complete boot lifetime and physical address space, dynamic map/unmap with TLB invalidation, recoverable guard-fault testing, SMP/per-CPU roots and VMX state, APIC/interrupt/guest-timer virtualization and scheduler-driven preemption, an independent host watchdog, and a general guest loader. Architectural-state blockers include XSAVE/FXSAVE, host FPU/SIMD preservation and context switching, lazy and multi-vCPU state, WRMSR PAT, MTRR/PAT/MMIO policy, SMP/per-CPU PAT, complete stateful MSR handling, and general exception injection. Devices and IOMMU isolation, live AMD/ARM backends, hardware soak, and a secure update/attestation/incident-response lifecycle also remain open. The fixed CPU-signature denylist and toy probes cannot rule out unknown timer, TSC, PAT, or FPU/SIMD errata.
